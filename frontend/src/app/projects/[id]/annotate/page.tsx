...
import { projectsApi, imagesApi, annotationsApi, segmentationApi, type Project, type Image, type Annotation, type ClassConfig, type SegmentationResult } from '@/lib/api';
...
export default function AnnotatePage({ params }: Props) {
  ...
  const handleCanvasClick = async (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    switch (activeTool) {
      case 'select':
        ...
        const clickedAnn = annotations.find((ann) => isPointInPolygon([x, y], ann.polygon));
  ...
  const saveAnnotation = async (polygon: number[][]) => {
    if (!currentImage || selectedClassId === null) {
      notifications.show({
        title: '오류',
        message: '클래스를 선택해주세요',
        color: 'red',
  ...
  const confirmSamMasks = async () => {
    for (const mask of samPendingMasks) {
      await saveAnnotation(mask.polygon);
    }
    clearSamPending();
  };
