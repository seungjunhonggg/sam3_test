...
export default function AnnotatePage({ params }: Props) {
  ...
  const {
    ...
    setSamPendingMasks,
    clearSamPending,
    samConfidenceThreshold,
    setSamConfidenceThreshold,
    // Mask painting
    classMasks,
    brushSize,
    maskEditMode,
  ...
  // SAM3 confidence threshold 변경 핸들러
  const handleConfidenceChange = async (value: number) => {
    setSamConfidenceThreshold(value);
    try {
      await segmentationApi.setConfidence(value);
      console.log('SAM3 confidence threshold updated:', value);
    } catch (error) {
    ...
    <Box className={classes.container} onKeyDown={handleKeyDown} tabIndex={0}>
      ...
      <Box className={classes.content}>
        ...
        <Box className={classes.canvasArea} ref={containerRef}>
          ...
          {/* SAM3 도구 설정 바 */}
          {(activeTool === 'sam_point' || activeTool === 'sam_box' || activeTool === 'sam_text') && (
            <Paper className={classes.textPromptBar} shadow="sm" p="sm">
              <Group gap="md">
                <Text size="sm" fw={500}>Confidence:</Text>
                <Slider
                  value={samConfidenceThreshold}
                ...
                />
                <Text size="sm" w={50}>{samConfidenceThreshold.toFixed(2)}</Text>
                {activeTool === 'sam_text' && (
                  <>
                    <Divider orientation="vertical" />
                    <TextInput
